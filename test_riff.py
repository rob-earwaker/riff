...
class Test_ChunkStream_create(TestCase):
    def test_returns_ChunkStream_instance(self):
        datastream = io.BytesIO(b'MockData')
        chunk = riff.ChunkStream.create('MOCK', 8, datastream)
        self.assertIsInstance(chunk, riff.ChunkStream)

    def test_no_error_with_empty_data_stream(self):
        datastream = io.BytesIO(b'')
        riff.ChunkStream.create('MOCK', 0, datastream)

    def test_does_not_read_data_stream(self):
        datastream = io.BytesIO(b'MockData')
        riff.ChunkStream.create('MOCK', 8, datastream)
...
class Test_Chunk_consumed(TestCase):
    ...
    def test_False_if_pad_byte_not_expected_and_not_consumed(self):
        ...
        chunk = riff.ChunkStream.create('MOCK', 11, datastream)
        chunk.data.skipall()
        self.assertFalse(chunk.consumed)
...
class Test_Chunk_data(TestCase):
    def test_is_ChunkDataStream_instance_after_creating_chunk(self):
        datastream = io.BytesIO(b'MockData')
        chunk = riff.ChunkStream.create('MOCK', 8, datastream)
        self.assertIsInstance(chunk.data, riff.ChunkDataStream)

    def test_is_ChunkDataStream_instance_after_reading_chunk_from_stream(self):
        stream = io.BytesIO(b'MOCK\x08\x00\x00\x00MockData')
        chunk = riff.ChunkStream.readfrom(stream)
        self.assertIsInstance(chunk.data, riff.ChunkDataStream)
...
class Test_Chunk_writeto(TestCase):
    def test_error_if_chunk_data_partially_consumed(self):
        ...
        chunk = riff.ChunkStream.readfrom(instream)
        chunk.data.skip(4)
    ...
    def test_writes_created_unpadded_chunk(self):
        ...
        chunk = riff.ChunkStream.create('MOCK', 4, datastream)
        outstream = io.BytesIO()
        chunk.writeto(outstream)
    ...
    def test_writes_zero_pad_byte_for_created_padded_chunk(self):
        ...
        chunk = riff.ChunkStream.create('MOCK', 3, datastream)
        outstream = io.BytesIO()
        chunk.writeto(outstream)
...
class Test_Chunk_repr(TestCase):
    def test_for_unpadded_chunk_read_from_stream(self):
        stream = io.BytesIO(b'MOCK\x08\x00\x00\x00MockData')
        chunk = riff.ChunkStream.readfrom(stream)
        self.assertEqual("riff.ChunkStream(id='MOCK', size=8)", repr(chunk))

    def test_for_padded_chunk_read_from_stream(self):
        stream = io.BytesIO(b'MOCK\x0b\x00\x00\x00MockDataOdd\x00')
        chunk = riff.ChunkStream.readfrom(stream)
        self.assertEqual("riff.ChunkStream(id='MOCK', size=11)", repr(chunk))
...
class Test_ChunkDataStream_consumed(TestCase):
    def test_False_if_data_not_fully_consumed(self):
        datastream = io.BytesIO(b'MockData')
        chunk = riff.ChunkStream.create('MOCK', 8, datastream)
        chunk.data.skip(4)
...
class Test_ChunkDataStream_position(TestCase):
...
class Test_ChunkDataStream_size(TestCase):
    def test_for_unpadded_chunk_read_from_stream(self):
        stream = io.BytesIO(b'MOCK\x08\x00\x00\x00MockData')
        chunk = riff.ChunkStream.readfrom(stream)
...
class Test_ChunkDataStream_read(TestCase):
...
class Test_ChunkDataStream_readall(TestCase):
...
class Test_ChunkDataStream_readover(TestCase):
    def test_position_not_advanced_for_negative_size(self):
        stream = io.BytesIO(b'MOCK\x0b\x00\x00\x00MockDataOdd\x00')
        chunk = riff.ChunkStream.readfrom(stream)
        position_before = chunk.data.position
...
class Test_ChunkDataStream_readoverall(TestCase):
...
class Test_ChunkDataStream_skip(TestCase):
    def test_position_not_advanced_for_negative_size(self):
        stream = io.BytesIO(b'MOCK\x0b\x00\x00\x00MockDataOdd\x00')
        chunk = riff.ChunkStream.readfrom(stream)
        position_before = chunk.data.position
...
class Test_ChunkDataStream_skipall(TestCase):
    def test_advances_position_to_end_from_start(self):
        datastream = io.BytesIO(b'MockDataOdd\x00')
        chunk = riff.ChunkStream.create('MOCK', 11, datastream)
        chunk.data.skipall()
...
class Test_ChunkDataStream_writeto(TestCase):
    def test_error_if_chunk_data_partially_consumed(self):
        instream = io.BytesIO(b'MOCK\x08\x00\x00\x00MockData')
        chunk = riff.ChunkStream.readfrom(instream)
        chunk.data.skip(4)
    ...
    def test_read_then_write_preserves_bytes(self):
        ...
        chunk = riff.ChunkStream.create('MOCK', 11, indatastream)
...
class Test_ChunkDataStream_repr(TestCase):
    def test_for_unpadded_chunk(self):
        datastream = io.BytesIO(b'MockData')
        chunk = riff.ChunkStream.create('MOCK', 8, datastream)
        self.assertEqual("riff.ChunkDataStream(size=8)", repr(chunk.data))

    def test_for_padded_chunk(self):
        datastream = io.BytesIO(b'MockDataOdd')
        chunk = riff.ChunkStream.create('MOCK', 11, datastream)
        self.assertEqual("riff.ChunkDataStream(size=11)", repr(chunk.data))
